...
type StudentPreTestsTabProps = {
  activities: StudentActivityItem[];
};

export function StudentPreTestsTab({ activities }: StudentPreTestsTabProps) {
  const pretests = activities.filter((a) => a.type === "PRE_TEST");
    ...
    <section className="mb-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="font-display text-3xl text-primary">Pre-Tests</h2>
        </div>
      </div>
      {pretests.length === 0 ? (
        <section className="quest-panel p-5 text-sm text-stone-foreground/75">No pre-tests assigned yet.</section>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {pretests.map((activity) => {
            ...
            return (
              <Link key={activity.id} to={activityLink} className="quest-panel p-4 transition-colors hover:border-primary/30">
                <div className="flex items-start gap-3">
                  <div className="rounded-lg bg-black/30 p-2"><BookOpen className="h-5 w-5 text-accent" /></div>
