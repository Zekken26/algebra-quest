...
type StudentAssignmentsTabProps = {
  activities: StudentActivityItem[];
};

export function StudentAssignmentsTab({ activities }: StudentAssignmentsTabProps) {
  const assignments = activities.filter((a) => a.type === "ASSIGNMENT");
    ...
    <section className="mb-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="font-display text-3xl text-primary">Assignments</h2>
        </div>
      </div>
      {assignments.length === 0 ? (
        <section className="quest-panel p-5 text-sm text-stone-foreground/75">No assignments assigned yet.</section>
      ) : (
            ...
            return (
              <Link key={activity.id} to={activityLink} className="quest-panel p-4 transition-colors hover:border-primary/30">
                <div className="flex items-start gap-3">
                  <div className="rounded-lg bg-black/30 p-2"><FileText className="h-5 w-5 text-primary" /></div>
